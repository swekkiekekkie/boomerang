...
type t =
  {
    desc             : d             ;
    mutable brz_form : Impl.t option ;
  }
...
let rec get_brz_form r =
  begin match r.brz_form with
    | Some b -> b
    | None ->
      let b =
        begin match r.desc with
          | CSet cs -> Impl.mk_cset cs
          | String s -> Impl.mk_string s
          | Alt (r1,r2) ->
            Impl.mk_alt
              (get_brz_form r1)
              (get_brz_form r2)
...
let epsilon = mk_t (String "")
let empty = mk_t Empty
let ascii_set = mk_t (CSet [min_code,max_ascii_code])
...
let rec from_brz_form (b:Impl.t) : t =
  begin match b.desc with
    ...
    | Alt ts ->
      List.fold_left
        (fun acc t -> mk_alt acc t)
        empty
        (List.map from_brz_form ts)
    | Rep (t,i1,i2o) ->
      begin match i2o with
        | None ->
          let t = from_brz_form t in
          mk_seq
            (mk_iter t i1 i1)
            (mk_star t)
        | Some i2 ->
...
let format_t r = Impl.format_t (get_brz_form r)

let string_of_char_code = Impl.string_of_char_code

let string_of_t r = Impl.string_of_t (get_brz_form r)

let is_empty r = Impl.is_empty (get_brz_form r)

let is_final r = Impl.is_final (get_brz_form r)

let is_singleton r = Impl.is_singleton (get_brz_form r)

let disjoint_cex r1 r2 =
  Impl.disjoint_cex
    (get_brz_form r1)
    (get_brz_form r2)
...
let match_sub_string r s i1 i2 =
  Impl.match_sub_string
    (get_brz_form r)
    s
    i1
    i2
...
let match_string_positions r s =
  Impl.match_string_positions
    (get_brz_form r)
    s

let match_string_reverse_positions r s =
  Impl.match_string_reverse_positions
    (get_brz_form r)
    s
...
let split_positions r1 r2 s =
  Impl.split_positions
    (get_brz_form r1)
    (get_brz_form r2)
    s

let bad_prefix_position r s =
  Impl.bad_prefix_position
    (get_brz_form r)
    s

let split_bad_prefix r s =
  Impl.split_bad_prefix
    (get_brz_form r)
    s

let seq_split r1 r2 s =
  Impl.seq_split
    (get_brz_form r1)
    (get_brz_form r2)
    s
...
let rank r =
  Impl.rank (get_brz_form r)

let rec subregexp_list
    (r:t)
  : t list =
  r::
  (begin match r.desc with
...
let rec to_optician_regexp r =
  begin match r.desc with
    ...
    | Inter _
    | Reverse _
    | Expand _ -> failwith "cannot handle complex regex in synth"
  end
